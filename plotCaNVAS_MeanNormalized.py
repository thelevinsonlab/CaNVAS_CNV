...
def createPlots(outputpath):
	...
	for f in files:
		...
		fig.update_yaxes(title_text="MeanLRROutLeft", row=1, col=2)
		fig.update_yaxes(title_text="MeanLRROutRight", row=1, col=3)
		fig.update_yaxes(title_text="BAFHomozygote", row=2, col=1)
		fig.update_yaxes(title_text="BAFDisomic", row=2, col=2)
		fig.update_yaxes(title_text="MeanLRROutLeft", row=2, col=3)
		...
		fig.update_layout(height=900, width=1800, title_text="Mean Normalized "+f)	
		fig.update_annotations(clicktoshow='onoff')		
		fig.write_html(outputpath+"/plots/mean_normalized/"+f+"mean_normalized_plot.html")
